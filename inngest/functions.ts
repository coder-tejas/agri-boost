...
export const GenerateCropYeild = inngest.createFunction(
  { id: "ai/generate-crop-yield", triggers: { event: "ai/generate-crop-yield" } },
  async ({ event, step }) => {
    const { userName, userEmail, soil_test_file, other_data } = event.data

    const uploadImageUrls = soil_test_file || null;
...
`
    ...
    await step.run("SaveToDB", async () => {
      try {
        await db.insert(userSoilAnalysis).values({
...
export const helloWorldlonger = inngest.createFunction(
  { id: "hello-worldlonger", triggers: { event: "test/hello.worldlonger" } },
  async ({ event, step }) => {
